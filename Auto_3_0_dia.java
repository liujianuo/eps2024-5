...
@Autonomous(name="60+0")
public class Auto_3_0 extends LinearOpMode{
    // Define all motors
    DcMotor rightback;
    DcMotor leftback;
    DcMotor rightfront;
    ...
    protected static final double buffertime = 0.1;
    ...
    */
    ...
    public void diPID(double ticksFB, double ticksRL, double time){ //time in seconds
        double targetFB = -ticksFB;
        double targetRL = -ticksRL
        double targettime = time;
        double lasterror = 0;
        double lastangle = 0;
        ElapsedTime PIDtime = new ElapsedTime();
        ...
        ///*
        while(CMPtime.seconds() < buffertime && CUMtime.seconds() < time*3){ 
            if(Math.abs(positionFB - targetFB) > 100 || Math.abs(positionRL - targetRL) > 100 || Math.abs(setangle - getangle()) > 0.5){
                CMPtime.reset();
            }
            double expectedposFB = Math.abs(targetFB) < Math.abs((CUMtime.seconds()/targettime)*targetFB) ? target : (CUMtime.seconds()/targettime)*targetFB;
            double delta = PIDtime.seconds();
            position = leftfront.getCurrentPosition() - leftback.getCurrentPosition() - referencepos;
    ...
    public void runOpMode(){
        ...
        fbPID(-500, 0.2); //back up
        
        linear.setPower(-1);
        rlPID(1300, 0.87); //line up 
        linear.setPower(0);
        ...
        rotPID(179, 0.5); //turn around
        linear.setPower(0.75);
        rlPID(-2300, 0.5);

        
        
        fbPID(1250, 0.8); // approach sub
        linear.setPower(0);
        ...
        fbPID(1800, 0.85); // approach sub
        linear.setPower(0);
        
        linear.setPower(-1);
        pause(0.6, "score"); //score
        linear.setPower(0);
