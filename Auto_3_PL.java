...
@Autonomous(name="60PL1")
public class Auto_3_copy extends LinearOpMode{
    // Define all motors
    DcMotor rightback;
    DcMotor leftback;
    DcMotor rightfront;
    ...
    protected static final double GLCLOSED = 0.52;
    protected static final double KP = 0.015;
    protected static final double KD = 0.00001;
    protected static final double KI = 0.0035;
    
    protected static final double KPAl = 0.03;
    protected static final double KDAl = 0;
    protected static final double KIAl = 0.04;

    protected static final double KPA = 0.05;
    protected static final double KDA = 0.0001;
    protected static final double KIA = 0.0001;
    protected static final double buffertime = 0.08;
    ...
    */
    ...
    public void fbPID(double ticks, double time){ //time in seconds
        ...
        CMPtime.reset();
        ///*
        while(CMPtime.seconds() < buffertime && CUMtime.seconds() < time*4.5){ 
    ...
    public void rlPID(double ticks, double time){ //time in seconds
        ...
        ///*
        while(CMPtime.seconds() < buffertime && CUMtime.seconds() < time*4.5){ 
            if(Math.abs(position - target) > 150 || Math.abs(setangle - getangle()) > 0.5){
                CMPtime.reset();
            }
            double expectedpos = Math.abs(target) < Math.abs((CUMtime.seconds()/targettime)*target) ? target : (CUMtime.seconds()/targettime)*target;
            double delta = PIDtime.seconds();
    ...
    public void runOpMode(){
        ...
        rotPID(180, 0.3); //turn around

        
        fbPID(750, 1); //approach HP
        ...
        rotPID(179, 0.3); //turn around
        linear.setPower(0.78);
        rlPID(-2000, 0.4);

        
        
        fbPID(1300, 0.45); // approach sub
        linear.setPower(0);
        ...
        pause(0.2, "buffer");
        rotPID(180, 0.3); //turn around
        linear.setPower(0.56);
        rlPID(-2500, 0.3);

        
        
        fbPID(1600, 0.45); // approach sub
        linear.setPower(0);
